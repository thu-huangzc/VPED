...
# @Command : python datasets/gen_head_helmet.py
# @Require : 请先运行detect_person.py生成yolov8检测人体结果的伪标签，再运行该文件
...
FILE_ROOT = Path("./datasets")

# 原始数据集
IMAGE_SET_ROOT = FILE_ROOT.joinpath("VOC2028/ImageSets/Main")  # 图片区分文件的路径
IMAGE_PATH = FILE_ROOT.joinpath("VOC2028/JPEGImages")  # 图片的位置
ANNOTATIONS_PATH = FILE_ROOT.joinpath("VOC2028/Annotations")  # 数据集标签文件的位置
LABELS_ROOT = FILE_ROOT.joinpath("VOC2028/Labels")  # 进行归一化之后的标签位置

# YOLO 需要的数据集形式的新数据集
DEST_IMAGES_PATH = Path("Safety_Helmet_Train_dataset/images")  # 区分训练集、测试集、验证集的图片目标路径
DEST_LABELS_PATH = Path("Safety_Helmet_Train_dataset/labels")  # 区分训练集、测试集、验证集的标签文件目标路径

def merge_data(yolov8_label_root, dataset_label_root):
    ...
    for file_name in yolo_file:
        ...
        file_path = os.path.join(yolov8_label_root, file_name)
...
if __name__ == '__main__':
    ...
    # 将JPEGImages的后缀名统一为.jpg
    for filename in os.listdir(IMAGE_PATH):
        file_path = os.path.join(IMAGE_PATH, filename)
        if os.path.isfile(file_path):
            name, ext = os.path.splitext(filename)
            if ext != ".jpg":
                new_file_path = os.path.join(IMAGE_PATH, f"{name}.jpg")
    ...
    # 合并标签
    print("Merging Label...")
    merge_data('./datasets/VOC2028/PersonPseudoLabels', './datasets/VOC2028/Labels')
